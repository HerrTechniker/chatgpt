...
const LEGACY_DIR = path.join(PUBLIC_DIR, 'legacy');
...
const legacyPreferredRoutes = new Set([
  '/',
  '/index.html',
  '/solutions',
  '/solutions.html',
...
const legacyRouteAliases = new Map([
  ...
  ['/de/', '/'],
  ['/de/index.html', '/'],
  ['/de/solutions', '/solutions'],
  ['/de/solutions.html', '/solutions'],
  ['/de/services', '/services'],
  ['/de/services.html', '/services'],
  ['/de/industries', '/industries'],
  ['/de/industries.html', '/industries'],
  ['/de/downloads', '/downloads'],
  ['/de/downloads.html', '/downloads'],
  ['/de/contact', '/contact'],
  ['/de/contact.html', '/contact'],
  ['/de/about', '/about'],
  ['/de/about.html', '/about'],
  ['/de/internships', '/internships'],
  ['/de/internships.html', '/internships'],
  ['/de/virtual-tour', '/virtual-tour'],
  ['/de/virtual-tour.html', '/virtual-tour'],
...
const germanRedirects = {
  '/': '/de/',
  '/index.html': '/de/',
  '/index': '/de/',
  '/solutions': '/de/solutions',
  '/solutions.html': '/de/solutions',
  '/services': '/de/services',
  '/services.html': '/de/services',
  '/industries': '/de/industries',
  '/industries.html': '/de/industries',
  '/downloads': '/de/downloads',
  '/downloads.html': '/de/downloads',
  '/contact': '/de/contact',
  '/contact.html': '/de/contact',
  '/about': '/de/about',
  '/about.html': '/de/about',
  '/internships': '/de/internships',
  ...
  '/de': '/de/',
  '/virtual-tour': '/de/virtual-tour',
  '/virtual-tour.html': '/de/virtual-tour'
};
...
const pageRoutes = {
  ...
  '/de/solutions': 'solutions.html',
  '/de/solutions.html': 'solutions.html',
  '/de/services': 'services.html',
  '/de/services.html': 'services.html',
  '/de/industries': 'industries.html',
  '/de/industries.html': 'industries.html',
  '/de/downloads': 'downloads.html',
  '/de/downloads.html': 'downloads.html',
  '/de/contact': 'contact.html',
  '/de/contact.html': 'contact.html',
  '/de/about': 'about.html',
  '/de/about.html': 'about.html',
  '/de/internships': 'internships.html',
  '/de/internships.html': 'internships.html',
  '/de/impressum': 'impressum.html',
  '/de/impressum.html': 'impressum.html',
  '/de/datenschutz': 'datenschutz.html',
  '/de/datenschutz.html': 'datenschutz.html',
  '/de/virtual-tour': 'virtual-tour.html',
  '/de/virtual-tour.html': 'virtual-tour.html',
  '/en': 'en/index.html',
  '/en/': 'en/index.html',
  '/en/index.html': 'en/index.html',
  '/en/solutions': 'en/solutions.html',
  '/en/services': 'en/services.html',
  '/en/industries': 'en/industries.html',
  '/en/downloads': 'en/downloads.html',
  '/en/contact': 'en/contact.html',
  '/en/about': 'en/about.html',
  '/en/internships': 'en/internships.html',
  '/en/internships.html': 'en/internships.html',
  '/en/virtual-tour': 'en/virtual-tour.html',
  '/en/virtual-tour.html': 'en/virtual-tour.html',
  '/en/impressum': 'en/impressum.html',
  '/en/imprint': 'en/impressum.html',
  '/en/impressum.html': 'en/impressum.html',
  '/en/privacy': 'en/datenschutz.html',
  '/en/datenschutz': 'en/datenschutz.html',
...
function sanitizeLegacySegment(segment) {
  if (!segment) {
    return '';
  }
  return segment
...
async function findLegacyPage(normalizedPath) {
  const canonicalPath = legacyRouteAliases.get(normalizedPath) || normalizedPath;

  if (!legacyPreferredRoutes.has(canonicalPath)) {
    return null;
  }

  const trimmed = canonicalPath === '/' ? '' : canonicalPath.replace(/^\/+/, '');
  const safeFragment = sanitizeLegacySegment(trimmed);
  const candidates = [];

  if (!safeFragment) {
    candidates.push(path.join(LEGACY_DIR, 'index.html'));
  } else {
    if (safeFragment.endsWith('.html')) {
      candidates.push(path.join(LEGACY_DIR, safeFragment));
      const withoutExt = safeFragment.replace(/\.html$/i, '');
      if (withoutExt) {
        candidates.push(path.join(LEGACY_DIR, withoutExt, 'index.html'));
      }
    } else {
      candidates.push(path.join(LEGACY_DIR, safeFragment, 'index.html'));
      candidates.push(path.join(LEGACY_DIR, `${safeFragment}.html`));
    }
  }

  for (const candidate of candidates) {
    if (!candidate.startsWith(LEGACY_DIR)) {
      continue;
    }
...
async function handleRequest(req, res) {
  ...
  const redirectTarget = germanRedirects[normalizedPath];
  if (redirectTarget && redirectTarget !== pathname) {
    const statusCode = normalizedPath === '/' ? 302 : 301;
  ...
  if (normalizedPath === '/legal' || normalizedPath === '/legal.html') {
    res.writeHead(301, { Location: '/de/impressum' });
    res.end();
    return;
  }
  ...
  const legacyFile = await findLegacyPage(normalizedPath);
  if (legacyFile) {
    await serveFile(res, legacyFile);
    return;
  }
  ...
  const safeRequested = sanitizeLegacySegment(requestedPath);
  const candidateRoots = [PUBLIC_DIR];
  if (await fileExists(LEGACY_DIR)) {
    candidateRoots.push(LEGACY_DIR);
  }

  for (const rootDir of candidateRoots) {
    const basePath = safeRequested ? path.join(rootDir, safeRequested) : rootDir;
