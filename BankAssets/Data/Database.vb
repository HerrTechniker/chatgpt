...
Namespace BankAssets.Data
    Public Class Database
        ...
        Private Sub EnsureColumn(connection As SqliteConnection, tableName As String, columnName As String, columnDefinition As String)
            Dim pragma = connection.CreateCommand()
            pragma.CommandText = $"PRAGMA table_info({tableName});"
            Using reader = pragma.ExecuteReader()
                While reader.Read()
                    If reader.GetString(1).Equals(columnName, StringComparison.OrdinalIgnoreCase) Then
                        Return
                    End If
                End While
            End Using

            Dim alter = connection.CreateCommand()
